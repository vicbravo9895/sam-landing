    ...
    <section ref={sectionRef} id="problem" aria-label="El problema y la solucion" className="py-24 lg:py-40 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid lg:grid-cols-2 gap-20 items-start">
          ...
          <div>
            ...
            <div className="space-y-3">
              {problems.map((problem, index) => (
                <div
                  key={problem.title}
                  className={`group flex items-start gap-4 rounded-2xl p-5 border border-transparent hover:border-destructive/15 hover:bg-destructive/[0.03] transition-[opacity,transform,background-color,border-color] duration-500 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
          ...
          {/* Solution */}
          <div className={`transition-[opacity,transform] duration-700 delay-300 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
            <div className="sticky top-32">
              ...
              <div className="space-y-4">
                {solutions.map((solution, index) => (
                  <div
                    key={solution}
                    className={`flex items-center gap-4 transition-[opacity,transform,background-color,border-color] duration-500 ${isVisible ? "opacity-100 translate-x-0" : "opacity-0 translate-x-4"}`}
              ...
              <div className={`mt-12 p-8 rounded-2xl bg-gradient-to-br from-primary/[0.06] to-secondary/[0.06] border border-primary/10 transition-[opacity,transform] duration-700 ${isVisible ? "opacity-100 scale-100" : "opacity-0 scale-95"}`} style={{ transitionDelay: "900ms" }}>
